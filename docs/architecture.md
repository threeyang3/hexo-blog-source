...
会话协议版本为 1。URL 只允许 HTTP、`127.0.0.1`、与记录一致的端口、根路径和唯一一个
48 位十六进制 `token` 参数；相对路径、外部主机、额外参数和重复令牌均拒绝。服务在成功
监听后原子写入会话文件，正常关闭时只删除仍属于当前 PID/URL 的记录。异常退出可能留下
旧文件，因此客户端必须把 `/api/status` 成功作为复用条件，而不能只检查文件存在。
