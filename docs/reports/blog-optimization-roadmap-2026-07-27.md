...
博客今天的优势来自早期“先写再说”：内容数量已经足够支撑分类、专题和搜索，不需要为了
...
> 本文写于较早时期，保留当时的判断与表达。作者今天的看法可能已经变化。
...
主动回到昨天版本的问题。公众号式后台真正让人安心的地方，是编辑过程中可以反复保存，
并且知道每个版本何时产生、改了哪些段落。

建议在不引入数据库的前提下，增加文章级历史：

- 每次保存前，把旧版本写入 `.blog-admin/history/<article-id>/`，只保留最近若干版。
...
不需要“编辑、主编、管理员”三套权限，但仍需要状态，因为未来的自己就是今天的审阅者。
